...
1. **Best approach**: Generate with a magenta background (see `generate-image:gemini` skill), then:
   ```bash
   # Sample actual background color from corner pixel:
   # from PIL import Image; print(Image.open("image.png").getpixel((0,0)))
   # Then use that color with feather (activates HSV chroma key + spill suppression):
   run.sh alpha <image> --transparent "253,1,252" --tolerance 15 --feather 40 -o <output>
   run.sh trim <output> -o <final>
   ```

2. **For existing white-background images**:
   ```bash
   run.sh alpha <image> --transparent "255,255,255" --tolerance 15 --feather 40 -o <output>
   ```

**Why `--feather` matters**: Without it, you get binary transparency with magenta/white halos on outlines. With it, the algorithm switches to HSV-based hue detection, Gaussian-blurred alpha edges, and spill suppression that removes background color contamination from outline pixels.

**Why magenta over white?** Magenta is far from most subject colors, giving cleaner spill suppression.
